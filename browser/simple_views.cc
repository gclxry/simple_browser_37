...
#include "content/simple/browser/simple_platform_data_aura.h"
...
namespace content {
  ...
  namespace {
    ...
    public:
      explicit ContextMenuModel(
        SimpleWebContentsDelegate* shell, const content::ContextMenuParams& params)
    ...
    private:
      ...
      SimpleWebContentsDelegate* shell_;
    ...
    public:
      ...
      ShellWindowDelegateView(SimpleWebContentsDelegate* shell)
        : shell_(shell),
  ...
  views::ViewsDelegate* SimpleWebContentsDelegate::views_delegate_ = NULL;

  // static
  void SimpleWebContentsDelegate::PlatformInitialize(const gfx::Size& default_window_size) {
#if defined(OS_WIN)
  ...
  void SimpleWebContentsDelegate::PlatformExit() {
  ...
  void SimpleWebContentsDelegate::PlatformCleanUp() {
  }

  void SimpleWebContentsDelegate::PlatformEnableUIControl(UIControl control, bool is_enabled) {
  ...
  void SimpleWebContentsDelegate::PlatformSetAddressBarURL(const GURL& url) {
    ShellWindowDelegateView* delegate_view =
      static_cast<ShellWindowDelegateView*>(window_widget_->widget_delegate());
    delegate_view->SetAddressBarURL(url);
  }

  void SimpleWebContentsDelegate::PlatformSetIsLoading(bool loading) {
  }

  void SimpleWebContentsDelegate::PlatformCreateWindow(int width, int height) {
  ...
  void SimpleWebContentsDelegate::Close() {
      window_widget_->CloseNow();
  }

  void SimpleWebContentsDelegate::PlatformSetTitle(const base::string16& title) {
    ShellWindowDelegateView* delegate_view =
      static_cast<ShellWindowDelegateView*>(window_widget_->widget_delegate());
  ...
  bool SimpleWebContentsDelegate::PlatformHandleContextMenu(
  ...
  void SimpleWebContentsDelegate::PlatformWebContentsFocused(WebContents* contents) {
    ShellWindowDelegateView* delegate_view =
      static_cast<ShellWindowDelegateView*>(window_widget_->widget_delegate());
